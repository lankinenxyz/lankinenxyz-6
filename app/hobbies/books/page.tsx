...
function BookCard({ book }: { book: Book }) {
  ...
  const className = "group relative block min-h-80 overflow-hidden border border-white/10 bg-white/[0.055] backdrop-blur transition hover:border-white/24";
  const content = (
    <>
...
function BookImage({ imageUrl, label }: { imageUrl: string | null; label: string }) {
  if (!imageUrl) {
    return <div className="grid min-h-80 place-items-center bg-black/24 font-mono text-xs uppercase tracking-[0.12em] text-white/34">No image</div>;
  }

  return <div aria-label={label} className="min-h-80 bg-cover bg-center transition duration-500 group-hover:scale-105" role="img" style={{ backgroundImage: `url(${imageUrl})` }} />;
}
