import type { OtherBlock, OtherRichText } from "@/lib/notion-other";

export default function OtherContent({ blocks }: { blocks: OtherBlock[] }) {
  if (blocks.length === 0) {
...
function BlockRenderer({ block }: { block: OtherBlock }) {
...
function NestedBlocks({ blocks }: { blocks: OtherBlock[] }) {
  return (
...
function renderRichText(richText: OtherRichText[]) {
